"""
process/log_decoder.py

To detect and decode payloads embedded in HTTP request query strings.
"""
...
class LogDecoder:
    ...
    # Change list to set for faster keyword lookup (O(1) lookup).
    ...
    def extract_base64_from_php_functions(self, s: str) -> List[str]:
        """
        Extract base64 strings from PHP patterns like:
        - base64_decode("...")
        - base64_decode('...')
        - base64_decode(\"...\")
        - base64_decode(\\\"...\\\")
        ...
        # Pattern for capturing various escape variations
        patterns = [
    ...
    def try_php_base64_decode(self, s: str) -> Optional[Tuple[str, str]]:
        """
        Try to extract and decode base64 from PHP functions.
        ...
        try:
            ...
            for key, vals in qs.items():
                for raw_val in vals:
                    ...
                    if self.CMD_START_DECODED.match(decoded_val):
                        frags = self.split_shell_like(decoded_val)
                        found_cmds.extend(frags if frags else [decoded_val])
                    else:
                        if isinstance(raw_val, str) and self.CMD_START_RAW.match(raw_val):
    ...
    def decode_wp_meta_from_msg(self, msg_raw: str) -> str:
        ...
        - If user_agent as tool/script -> do decoding
        - Run detect_rce_query_simple() ro add [RCE: "..."] if detected
        """
        original = msg_raw
        ...
        # --- Check PHP base64_decode pattern first ---
        php_decode = self.try_php_base64_decode(msg_raw)
        if php_decode:
            b64_original, b64_decoded = php_decode
            # Check if the decode result contains command keywords
        ...
        # --- If no decode results yet, try the old method ---
        if not decoded_annotation:
            ...
            bad_agents = ["python", "perl", "postman", "curl", "wget", "go-http-client", "java", "shell", "httpclient", "mozilla/5.0"]
