"""
process/low_level_predict.py

Semantic labeling.
To predict low-level semantic labels from logs in the timeline.
"""
...
# ── Log Type Detection (Enhanced) ────────────────────────────────────────────

def detect_log_type(desc: str, filename: str) -> Dict[str, Any]:
    """
    Detect log type and extract important fields.
    """
    parsed = {}
    ...
    # 4. If the log type is not recognized, try automatic detection based on content
    else:
        log_types = _auto_detect_log_type(desc, filename)
        # If it looks like an HTTP log, extract HTTP fields
        if _looks_like_http_log(desc):
            _extract_http_fields(desc, parsed)

    # Fallback - generic type can match with many rule
    if not log_types:
...
class SigmaMatcher:
    ...
    def match(self, log_entry: Dict[str, Any]) -> bool:
        """Check if log entry match with rule"""
        if not log_entry:
            return False
    ...
    def _check_logsource(self, log_entry: Dict[str, Any]) -> bool:
        """
        Check if the log_entry matches the expected logsource for the rule.
    ...
    def _match_selection(self, selection, log_entry: Dict) -> bool:
        """
        Match selection against the log entry.
        In flexible mode, always search in all possible fields.
        """
    ...
    def _get_search_fields(self, log_entry: Dict) -> List[str]:
        ...
        # Include HTTP-related fields if they exist
    ...
    def _get_field_value(self, log_entry: Dict, field_name: str) -> str:
        ...
        If the field does not exist, fallback to desc.
        """
        # Direct match
    ...
    def _parse_field(self, field: str):
        """
        Parse field name and modifier from the Sigma rule field.
    ...
    def _evaluate_condition(self, condition: str, selections: Dict[str, bool]) -> bool:
        ...
        try:
            ...
            # Dont print warning for each row
            return any(selections.values())
...
def extract_sigma_priority(sigma_value: str):
    """
    Pick the highest priority rule based on severity.
...
class LowLevelPredict:
    def __init__(self, dataset):
        ...
        # Set True for flexible mode (all rules are tried for all logs)
        # Set False for strict mode (only rules matching the log_type are tried)
        self.FLEXIBLE_MODE = False
        ...
        with open(self.CSV_INPUT, newline='', encoding="utf-8", errors='replace') as f, \
            ...
            for row in reader:
                ...
                if processed % 10000 == 0:
                    ...
                    # Format total elapsed to minute:second
