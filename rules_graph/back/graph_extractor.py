...
class GraphExtractor:
    ...
    def process_row(self, row: list):
        ...
        if template.log_type in ("access.log", "web_log"):
            fields["actor"] = fields.get("source_ip", "")
            # Cek matched username dari payload
            ...
            # PAM: pakai target_user sebagai effective username
            eff_user = fields.get("username") or (
                fields.get("target_user", "")
                if fields.get("auth_type", "").startswith("pam_") else ""
            )
            fields["username"] = eff_user
            fields["actor"]    = eff_user or fields.get("source_ip", "")
        elif template.log_type == "syslog":
            fields["actor"] = fields.get("extracted_username") or fields.get("service", "")
    ...
    def _execute(self, template: ExtractionTemplate, fields: dict):
        ...
        if template.log_type in ("access.log", "web_log"):
            ...
            branch = "with_user" if fields.get("username") else "orphan"
        elif template.log_type == "syslog":
